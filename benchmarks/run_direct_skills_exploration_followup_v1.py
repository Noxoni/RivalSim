"""Bounded +600 -> +650 same-lineage, temperature-only PPO continuation."""
...
VERSION = "RIVAL2_DIRECT_SKILLS_EXPLORATION_T2_FOLLOWUP_V1"
PRIOR_AMENDMENT_SHA = "CE9E6EB5DDC1F4BD50975F47C0E1FFF7A0D3198FA9C24C254CA6A9C79B1A45D4"

OUT = base.RESULTS / "exploration_t2_followup_v1"
SOURCE = base.CHECKPOINTS / "plus_000600.pt"
SOURCE_SHA = "8AC192B4154223DB7CE82B41C77DC475D63B6AFB260EF0B9B9AC4455F6C6F6B2"
START, REVIEW = 600, 650
SOURCES = (
    "benchmarks/run_direct_skills_exploration_followup_v1.py",
    "rivalsim/direct_skills_exploration_v1.py",
    "tests/test_direct_skills_exploration_followup.py",
)
...
def amendment():
    return dict(
        ...
        sole_learning_change="None relative to accepted600. Continue temperature2 with identical "
        "sampling/likelihood, rewards, data mixture, architecture and PPO",
        prior_amendment_sha256=PRIOR_AMENDMENT_SHA,
        decision_evidence="EVALUATION_000600.md;65/144 vs62/163 at550,7/10 matched cases improve; "
        "still0wins,finishing6/64. Bounded50-update follow-up, not proof of causality",
        ...
        initialization="Exact latest accepted600 model/Adam/RNG; no fresh optimizer or actor",
        ...
        reward_change=False,
        critic_change=False,
        checkpoints="Rolling every accepted update; permanent650 and pause boundary",
        evaluations="Unchanged64cases/family and ten regulation matches at650, corrected contact "
        "cache reset method. Compare600 and550; same cases/seeds, no best-checkpoint selection",
        end="Stop at650 after evaluation for goal-agent review, not cancellation of user goal. "
        ...
        diagnostic="Completed LEARNING_SIGNAL_000550.md; do not repeat",
    )
...
def preflight():
    ...
    checks = dict(
        ...
        no_optimizer_steps={int(s["step"]) for s in optimizer.state.values()} == {137558},
...
def validate_latest(path, expected, *, external=base.EXTERNAL):
    ...
    if offset == REVIEW and state.get("status") == "stopped_for_exploration_review":
    ...
    # the new run's offset600 status. No later state can claim the prior block.
    ...
    try:
        ...
        if offset == REVIEW:
            ...
            if not marker.exists():
                marker.write_text(
                    "Agent review boundary650: temperature2 experiment complete. "
        ...
        write_json(base.EXTERNAL / "failure_exploration_followup.json", failure)
