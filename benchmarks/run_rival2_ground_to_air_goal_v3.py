"""Train goalward repeated contacts after Rival's validated self-created pop.

V2 established a reliable pop-to-elevated-recontact chain.  V3 keeps that
literal entry and trains the same canonical option to remain connected, add
goalward momentum on airborne contacts, and score before the ball lands.
"""
...
from rivalsim.rival2_ground_to_air_goal_v3 import (  # noqa: E402
...
AUTHORITY_SHA256 = "E0E5786EDAF1107FD6A24A4345417F6BFDB3A0D8E49176D8728B71F2A40A66B8"
...
PARENT_SHA256 = "FEF43A01FF627B4B41BFA76AED811BD21F2A4E8900B1A34FFE0A736E12C71249"
...
DEFAULT_RUN_DIR = Path("G:/dev/RivalSim-runs/ground-to-air-goal-v3")
...
class GoalDirectedTrainingTracker:
    """Training-only physical reward for a connected, goalward aerial chain."""
    ...
    def step(
        ...
        *,
        ...
        ball_goalward_progress = (
            after[:, self.side, FIELD["ball.position.y"]]
        ...
        ground_failure = (
            ...
            & (tick >= int(self.authority["episode"]["ground_failure_after_tick"]))
...
def collect_rollout(
    ...
    *,
    ...
    tracker = GoalDirectedTrainingTracker(
        worlds,
        attacker_side=side,
        horizon=horizon,
        authority=authority,
...
def training_phase_for_block(block: int, authority: dict[str, Any]) -> int:
    selected = int(authority["training"]["phase_schedule"][0]["phase"])
    for row in authority["training"]["phase_schedule"]:
        if int(row["first_block"]) > block:
            break
        selected = int(row["phase"])
    return selected
...
def successful_rehearsal_update(
    ...
    *,
    authority: dict[str, Any],
    generator: torch.Generator,
    rehearsal_key: str = "successful_rehearsal",
) -> dict[str, Any]:
    """Rehearse only actions that physically preceded an elevated recontact."""

    rehearsal = authority["training"][rehearsal_key]
...
def evaluation_score(rows: list[dict[str, Any]]) -> float:
    ...
    return float(
        ...
        + minimum["goalward_velocity_contact"]
...
def save_checkpoint(
    ...
    *,
    ...
    payload["optimizer"] = {
        "format": "RIVAL2_GROUND_TO_AIR_GOAL_V3_FRESH_ADAMW",
        "option": optimizer.state_dict(),
    }
    payload["curriculum_transition"] = {
        "identity": GROUND_TO_AIR_GOAL_V3_VERSION,
        "created_utc": utc_now(),
        "authority": {
            "path": AUTHORITY.relative_to(ROOT).as_posix(),
            "sha256": AUTHORITY_SHA256,
        },
        "passing_ground_to_air_v2_parent_sha256": PARENT_SHA256,
        ...
        "critic_frozen": True,
        "goal_directed_physical_trajectory_rehearsal": True,
...
def run(args: argparse.Namespace) -> int:
    ...
    preflight = {
        "format": "RIVAL2_GROUND_TO_AIR_GOAL_V3_PREFLIGHT",
        "created_utc": utc_now(),
        "authority_sha256": AUTHORITY_SHA256,
        "diagnostic_parent_hash_verified": True,
        "protected_v23_hashes_verified": True,
        "canonical_shared_policy": True,
        "full_chain_goal_validation": True,
        "critic_frozen": True,
        "production_reward_unchanged": True,
    ...
    for block in range(1, maximum_blocks + 1):
        ...
        training_phase = training_phase_for_block(block, authority)
        sides = []
        try:
            for side in (0, 1):
                ...
                goal_rehearsal = successful_rehearsal_update(
                    model,
                    optimizer,
                    rollout,
                    authority=authority,
    ...
    if controlled_pass:
        ...
        checkpoints.append(
            save_checkpoint(
                ...
                model,
                optimizer,
                CHECKPOINTS / "rival2_ground_to_air_goal_v3.pt",
                block=best_block,
                evaluation={"validation": best_evaluation, "test": test},
            )
        )
    result = {
        "format": "RIVAL2_GROUND_TO_AIR_GOAL_V3_RESULT",
        "created_utc": utc_now(),
        "authority_sha256": AUTHORITY_SHA256,
        "baseline_validation": baseline,
        "best_block": best_block,
