"""Fresh full-scenario restart: +1 first touch, +.25 airborne-ball contacts, sustained rewards retained."""
...
from rivalsim.fresh_sustained_contact_v1 import (VERSION, SEED, MAX_UPDATES, WORLDS,
    training_bank, new_model, authority, ContactEnv as AcquisitionEnv, ContactCollector as AcquisitionCollector)
from rivalsim.sustained_acquisition_v1 import FAMILY, acquisition_starts, SEED as BANK_SEED
...
OUT = ROOT / "results/rival2/fresh_sustained_contact_v1"
CKPTS = ROOT / "checkpoints/rival2/fresh_sustained_contact_v1"
EXTERNAL = Path("G:/dev/RivalSim-runs/fresh-sustained-contact-v1")
...
SOURCES = (*BASE_SOURCES, "rivalsim/sustained_acquisition_v1.py",
    ...
    "benchmarks/run_fresh_sustained_contact_v1.py", "rivalsim/fresh_sustained_contact_v1.py",
    "tests/test_fresh_sustained_contact_v1.py", "tests/test_fresh_acquisition_only_v1.py", "rivalsim/sustained_standing_kickoff_v1.py",
    "tests/test_sustained_standing_kickoff_v1.py", "tests/test_fresh_acquisition_touch_v1.py",
    "rivalsim/fresh_acquisition_touch_v1.py", "rivalsim/static_world.py",
    "rivalsim/kernels/ball_world.py", "rivalsim/ball_world_state.py")
...
def preflight():
    ...
    checks = dict(model_unchanged=before==tensor_hash(model.state_dict()), optimizer_empty=not optimizer.state,
        ...
        native_first_touch_bonus_present=collector.last_metrics["contact_bonuses"]["learner_first_awards"]>0,
        native_air_touch_bonus_present=collector.last_metrics["contact_bonuses"]["learner_air_awards"]>0,
        critic_isolated=isolated, all_six_families=set(env.family.cpu().tolist())==set(range(6)),
        all_six_exposed=all(sum(v["world_seconds"] for k,v in exposure.items() if k.startswith(name+"_"))>0 for name in ("natural","challenge","finishing","defense","kickoff","ball_acquisition")),
...
def run(args):
    ...
    def save(path):
        return atomic_checkpoint(path,dict(format=VERSION+"_CHECKPOINT",policy_version=POLICY_VERSION,parent=None,seed=SEED,
            ...
            last_metrics=collector.last_metrics,first_touch_latches=dict(shape=list(env.bonus_views["first_paid"].shape),
            paid=int(env.bonus_views["first_paid"].sum()),sha256=tensor_hash({"first":env.bonus_views["first_paid"],"air":env.bonus_views["air_paid"]}),
            resume="Fresh physical episodes clear eligibility latches; no duplicate payout in an in-process episode."),recurrent_state_audit=dict(shape=list(collector.hidden.shape),
    ...
    try:
        ...
        while steps<MAX_UPDATES and not (EXTERNAL/"STOP").exists():
            ...
            if not set(env.family.cpu().tolist()).issubset(set(range(6))):
                raise RuntimeError("Unexpected scenario family")
...
def report(offset):
    ...
    checks=dict(parent_none=ck["parent"] is None,authority=ck["authority_sha256"]==content_hash(authority()),
        ...
        contact_accounting=all(all(abs(r["training"]["potential_reward_components"][key]-r["training"]["contact_bonuses"][name])<1e-5 for key,name in (("first_touch","learner_first_reward"),("air_touch","learner_air_reward"))) for r in rows),
        ...
        all_six_exposed=all(sum(v["world_seconds"] for k,v in r["training"]["by_start_family_and_opponent"].items() if k.startswith(name+"_"))>0 for r in rows for name in ("natural","challenge","finishing","defense","kickoff","ball_acquisition")),
