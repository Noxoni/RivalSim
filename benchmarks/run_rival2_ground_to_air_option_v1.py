"""Train a side-specific post-pop controller in a bounded physical shot pack.

The protected V23 policies remain unchanged.  A fixed observation-only
possession/pop/neutral-jump bootstrap creates the opportunity, then a separate
option policy learns the post-jump continuation.  The training reward contains
no named-mechanic classifier and no payment for raw airtime.
"""
...
AUTHORITY = ROOT / "results/rival2/ground_to_air_option_v1/authority.json"
AUTHORITY_SHA256 = "87124D3C5FC3ADB7097CFB5CF9F34A1856679418DD9A93C7D266DC51B857BAA8"
RESULTS = ROOT / "results/rival2/ground_to_air_option_v1"
CHECKPOINTS = ROOT / "checkpoints/rival2/ground_to_air_option_v1"
...
DEFAULT_RUN_DIR = Path("G:/dev/RivalSim-runs/ground-to-air-option-v1")
...
def collect_rollout(
    ...
    *,
    ...
    config = GroundToAirConfig(**authority["bootstrap_config"], learned_after_second_jump=True)
    ...
    for tick in range(horizon):
        ...
        if rollout is not None:
            ...
            rollout.mask[tick].copy_(active_before & option.learned_control)
...
def save_checkpoint(
    ...
    *,
    ...
    payload["optimizer"] = {
        "format": "RIVAL2_GROUND_TO_AIR_OPTION_V1_FRESH_ADAMW",
        "option": optimizer.state_dict(),
    }
    payload["curriculum_transition"] = {
        "identity": GROUND_TO_AIR_OPTION_VERSION,
        "created_utc": utc_now(),
        "authority": {
            "path": AUTHORITY.relative_to(ROOT).as_posix(),
            "sha256": AUTHORITY_SHA256,
        },
        "protected_parent_sha256": BLUE_SHA256 if side == 0 else ORANGE_SHA256,
...
def run(args: argparse.Namespace) -> int:
    ...
    source_paths = (BLUE, ORANGE)
    expected_hashes = (BLUE_SHA256, ORANGE_SHA256)
    sources = []
    for path, expected in zip(source_paths, expected_hashes, strict=True):
        if capability.sha256_file(path) != expected:
            raise RuntimeError(f"protected V23 checkpoint changed: {path}")
        sources.append(torch.load(path, map_location="cpu", weights_only=False))
    models = [make_model(source, args.device) for source in sources]
    optimizers = [make_optimizer(model, authority) for model in models]
    critic_hashes = [
        autonomous.tensor_tree_sha256(
    ...
    preflight = {
        "format": "RIVAL2_GROUND_TO_AIR_OPTION_V1_PREFLIGHT",
        "created_utc": utc_now(),
    ...
    for block in range(1, maximum_blocks + 1):
        ...
        for side in (0, 1):
            ...
            del rollout
            sides.append({"side": side, "rollout": rollout_metrics, "ppo": ppo})
    ...
    for side in (0, 1):
        models[side].load_state_dict(best_models[side], strict=True)
    ...
    # The test seed remains untouched until the validation-selected model is frozen.
    test = [
        collect_rollout(
            models[side],
            geometry,
    ...
    controlled_pass = passes_gate(best_evaluation, authority) and passes_gate(test, authority)
    checkpoints = []
    if controlled_pass:
        ...
        for side in (0, 1):
            checkpoints.append(
                save_checkpoint(
                    ...
                    models[side],
                    optimizers[side],
                    CHECKPOINTS / ("rival2_blue.pt" if side == 0 else "rival2_orange.pt"),
                    side=side,
                    block=best_block,
    ...
    result = {
        "format": "RIVAL2_GROUND_TO_AIR_OPTION_V1_RESULT",
        "created_utc": utc_now(),
        "authority_sha256": AUTHORITY_SHA256,
        "baseline_validation": baseline,
        "best_block": best_block,
        ...
        "protected_v23_unchanged": all(
            capability.sha256_file(path) == expected
            for path, expected in zip(source_paths, expected_hashes, strict=True)
        ),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--worlds-per-side", type=int, default=2_048)
    ...
    parser.add_argument("--maximum-blocks", type=int, default=80)
