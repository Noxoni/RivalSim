"""Fresh acquisition-only restart: explicit +1 own-first-touch, sustained rewards retained."""
...
from rivalsim.fresh_acquisition_touch_v1 import (VERSION, SEED, MAX_UPDATES, WORLDS,
    training_bank, new_model, authority, FirstTouchEnv as AcquisitionEnv, FirstTouchCollector as AcquisitionCollector)
...
OUT = ROOT / "results/rival2/fresh_acquisition_touch_v1"
CKPTS = ROOT / "checkpoints/rival2/fresh_acquisition_touch_v1"
EXTERNAL = Path("G:/dev/RivalSim-runs/fresh-acquisition-touch-v1")
...
SOURCES = (*BASE_SOURCES, "rivalsim/sustained_acquisition_v1.py",
    ...
    "benchmarks/run_fresh_acquisition_touch_v1.py", "rivalsim/fresh_acquisition_touch_v1.py",
    "tests/test_fresh_acquisition_touch_v1.py", "tests/test_fresh_acquisition_only_v1.py")
...
def preflight():
    ...
    checks = dict(model_unchanged=before==tensor_hash(model.state_dict()), optimizer_empty=not optimizer.state,
        ...
        native_first_touch_bonus_present=collector.last_metrics["first_touch_bonus"]["learner_awards"]>0,
...
def run(args):
    ...
    def save(path):
        return atomic_checkpoint(path,dict(format=VERSION+"_CHECKPOINT",policy_version=POLICY_VERSION,parent=None,seed=SEED,
            ...
            last_metrics=collector.last_metrics,first_touch_latches=dict(shape=list(env.first_touch_paid.shape),
            paid=int(env.first_touch_paid.sum()),sha256=tensor_hash({"paid":env.first_touch_paid}),
...
def report(offset):
    ...
    checks=dict(parent_none=ck["parent"] is None,authority=ck["authority_sha256"]==content_hash(authority()),
        ...
        first_touch_accounting=all(abs(r["training"]["potential_reward_components"]["first_touch"]-r["training"]["first_touch_bonus"]["learner_reward_sum"])<1e-5 for r in rows),
