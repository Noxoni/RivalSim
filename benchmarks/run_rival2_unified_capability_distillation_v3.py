"""DAgger-style natural closed-loop correction for unified Rival V2."""
...
AUTHORITY = ROOT / "results/rival2/unified_capability_distillation_v3/authority.json"
RESULTS = ROOT / "results/rival2/unified_capability_distillation_v3"
CHECKPOINT = (
    ROOT / "checkpoints/rival2/unified_capability_distillation_v3/rival2_unified_capability_v3.pt"
)
DEFAULT_RUN_DIR = Path("G:/dev/RivalSim-runs/unified-capability-distillation-v3")
FAMILY_STUDENT_NATURAL = "student_natural_v2"
FAMILIES = (
    v1.FAMILY_NATURAL,
...
def load_authority() -> dict[str, Any]:
    ...
    if authority.get("format") != "RIVAL2_UNIFIED_CAPABILITY_DISTILLATION_V3_AUTHORITY":
        raise RuntimeError("unified capability V3 authority format mismatch")
    if authority["integrity"]["optimizer_steps_before_authority_commit"] != 0:
        raise RuntimeError("V3 authority does not preserve prospective optimization")
    ...
    if committed != AUTHORITY.read_bytes():
        raise RuntimeError("V3 authority is not byte-identical to the committed object")
    identities = [*authority["sources"].values(), authority["parent"]]
...
def load_parent(
    authority: dict[str, Any], device: str
) -> tuple[dict[str, Any], Rival2UnifiedActorCritic]:
    payload = torch.load(ROOT / authority["parent"]["path"], map_location="cpu", weights_only=False)
    if payload.get("format") != authority["parent"]["format"]:
        raise RuntimeError("unified V2 parent format mismatch")
    config = Rival2UnifiedPolicyConfig(**payload["policy_config"])
    if payload.get("policy_config_sha256") != config.content_hash:
        raise RuntimeError("unified V2 parent config mismatch")
...
def student_natural_path(run_dir: Path, split: str) -> Path:
    return run_dir / "corpora" / split / "student_natural.pt"


def collect_student_natural(
    ...
    *,
    ...
    local = np.arange(worlds, dtype=np.int32)
    side = np.resize(np.asarray((0, 1), dtype=np.int32), worlds)
    layout = local % 5
    rng = np.random.default_rng(seed)
    permutation = rng.permutation(worlds)
    side = side[permutation]
    layout = layout[permutation]
    runner = physical.UnifiedNextoRunner(
        worlds,
        collision_root,
        parent,
    ...
    reset_before = torch.zeros(worlds, dtype=torch.bool, device=device)
    for tick in range(ticks):
        observation[tick].copy_(runner.observation[runner.batch, runner.rival_side].detach().cpu())
        valid[tick].copy_((~reset_before).cpu())
        runner.tick()
        reset_before.copy_(runner.last_reset_mask)
    reset_boundaries = int((~valid).sum())
    capability.activate_fresh_persistent_stream(device)
    del runner
    gc.collect()
    torch.cuda.empty_cache()
    return {
        "format": "RIVAL2_UNIFIED_STUDENT_NATURAL_CORPUS_V1",
        "kind": "student_natural_v2",
        "seed": seed,
        "observation": observation,
        "valid": valid,
        "side": torch.from_numpy(side.astype(np.int64)),
        "reset_boundary_samples_excluded": reset_boundaries,
        "opponent": "pinned Nexto",
        "source_policy_sha256": v1.sha256_file(
            ROOT / json.loads(AUTHORITY.read_text(encoding="utf-8"))["parent"]["path"]
        ),
    }
...
def build_all_corpora(
    ...
    *,
    ...
    for split in ("train", "validation"):
        ...
        corpus = collect_student_natural(
            parent,
            collision_root=collision_root,
            worlds=int(spec["student_natural_worlds"]),
            ticks=int(spec["student_natural_ticks"]),
            seed=int(authority["seeds"][f"{split}_student_natural"]),
            device=device,
        )
        row = v1.save_corpus(student_natural_path(run_dir, split), corpus)
        row["reset_boundary_samples_excluded"] = corpus["reset_boundary_samples_excluded"]
        manifest["splits"][split]["student_natural"] = row
        del corpus
    manifest["format"] = "RIVAL2_UNIFIED_CORPUS_MANIFEST_V3"
...
def load_pools(run_dir: Path, split: str, sequence_ticks: int) -> dict[str, v1.SequencePool]:
    pools = v1.load_pools(run_dir, split, sequence_ticks)
    student = torch.load(
        student_natural_path(run_dir, split), map_location="cpu", weights_only=False
    )
    pools[FAMILY_STUDENT_NATURAL] = v1.SequencePool.from_payload(
        student, sequence_ticks=sequence_ticks
    )
    return pools
...
def target_actor(
    ...
    teacher_family = v1.FAMILY_NATURAL if family == FAMILY_STUDENT_NATURAL else family
...
def eligible(metrics: dict[str, Any], authority: dict[str, Any]) -> bool:
    ...
    if metrics[v1.FAMILY_NATURAL]["expected_action_rmse"] > float(
        ...
        return False
    if metrics[FAMILY_STUDENT_NATURAL]["expected_action_rmse"] > float(
        selection["student_natural_expected_action_rmse_max"]
    ):
        return False
...
def save_checkpoint(
    ...
    *,
    ...
    payload = {
        "format": "RIVAL2_UNIFIED_CAPABILITY_CHECKPOINT_V3",
        ...
        "cumulative_supervised_steps": int(parent_payload["accepted_supervised_steps"]) + step,
...
def run(args: argparse.Namespace) -> int:
    ...
    parent_payload, model = load_parent(authority, args.device)
    _teacher_payloads, teachers = v1.load_teachers(authority, args.device)
    model.eval()
    preflight = {
        "format": "RIVAL2_UNIFIED_CAPABILITY_DISTILLATION_V3_PREFLIGHT",
        "created_utc": v1.utc_now(),
    ...
    if not preflight["frozen_base"]:
        raise RuntimeError("unified V3 base is not frozen")
    ...
    if run_dir.exists() and any(run_dir.iterdir()) and not args.resume_corpora:
        raise RuntimeError("unified V3 requires a fresh run directory")
    ...
    for step in range(1, maximum_steps + 1):
        ...
        if not bool(torch.isfinite(total) and torch.isfinite(gradient)):
            raise RuntimeError("nonfinite unified V3 loss or gradient")
        optimizer.step()
        if not all(bool(torch.isfinite(parameter).all()) for parameter in model.context_parameters):
            raise RuntimeError("nonfinite unified V3 context parameter")
        ...
        print(
            ...
            f"dagger={validation[FAMILY_STUDENT_NATURAL]['expected_action_rmse']:.6f} "
    ...
    if best_state is None or best_optimizer is None or best_metrics is None:
        result = {
            "format": "RIVAL2_UNIFIED_CAPABILITY_DISTILLATION_V3_RESULT",
            "created_utc": v1.utc_now(),
            "verdict": "BLOCKED",
            "reason": "no validation-eligible V3 checkpoint",
            "baseline": baseline,
