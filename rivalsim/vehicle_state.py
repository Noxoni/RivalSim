...
class VehicleState:
    ...
    def __init__(self, car_count: int, device: str):
        ...
        self.contact_point = wp.zeros(
            self.contact_capacity, dtype=wp.vec3, device=device
        )
        self.contact_normal = wp.zeros(
            self.contact_capacity, dtype=wp.vec3, device=device
        )
        self.contact_penetration = wp.zeros(
            self.contact_capacity, dtype=wp.float32, device=device
        )

    @property
    def logical_bytes(self) -> int:
        vec3_count = self.wheel_count * 4 + self.contact_capacity * 2
        float_count = self.wheel_count * 12 + self.car_count + self.contact_capacity
        int_count = self.wheel_count * 2 + self.car_count * 3
    ...
    def snapshot(self) -> VehicleSnapshot:
        ...
        return VehicleSnapshot(
            ...
            suspension_clipped_factor=array(
                "suspension_clipped_factor", np.float32, wheel_shape
            ),
            ...
            longitudinal_friction=array(
                "longitudinal_friction", np.float32, wheel_shape
            ),
            ...
            contact_penetration=array(
                "contact_penetration", np.float32, contact_shape
            ),
        )
